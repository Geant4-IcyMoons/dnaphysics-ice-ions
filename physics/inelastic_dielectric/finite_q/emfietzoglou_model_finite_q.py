...
from physics.inelastic_dielectric.k_shell.hydrogenic import (
    oxygen_K_hydrogenic_gos_df_dE,
    _oxygen_K_hydrogenic_strength_table,
    oxygen_K_hydrogenic_gos_continuum_strength,
    oxygen_K_hydrogenic_occupancy_remainder,
    oxygen_K_hydrogenic_gos_elf,
    oxygen_K_ion_hydrogenic_gos_elf,
    oxygen_K_hydrogenic_gos_fsum,
    OXYGEN_K_B_EV,
    OXYGEN_K_ZEFF,
    OXYGEN_K_OCCUPANCY,
    OXYGEN_K_GOS_VERSION,
    RYD_ELECTRON_VOLT
)
...
def default_dispersion_coefficients():
    """Return the shared finite-q dispersion parameters used by table generators."""
    return DispersionCoeffs(
        a_fj=np.array(DEFAULT_A_FJ),
        b_fj=np.array(DEFAULT_B_FJ),
        c_fj=np.array(DEFAULT_C_FJ),
    )
...
def _renorm_fi_q(
    ...
    fj0_sum0: float,
    fjq_sum: np.ndarray,
    inner_shell_strength_electrons=None,
) -> np.ndarray:
    """
    Outer-ionization strengths from the finite-q f-sum allocation.

    With no explicit inner-shell strength, retain the historical electron
    model exactly:
      f_i(q) = f_i * (1 - sum_j f_j(q)) / (1 - sum_j f_j(0)).

    For the ion hydrogenic-GOS path, the independently calculated K-continuum
    strength is included in the same molecular sum-rule budget:
      sum_i f_i(q) = 1 - sum_j f_j(q) - F_K,cont(q)/10.
    This applies the S1 constraint to all three terms in the decomposition of
    Dingfelder, Appl. Radiat. Isot. 83, 142 (2014), Eqs. (13)-(26); omitting
    F_K from the budget allocates 10 electrons before the K term is added.
    This preserves the K GOS amplitude and the relative strengths of the four
    physical outer-ionization channels. It is a joint channel allocation, not
    an a posteriori multiplier applied to the valence ELF or K continuum.
    Returns shape (nq, n_ion).
    """
    fi0 = np.asarray(fi0, float)
    fjq_sum = np.asarray(fjq_sum, float)
    if inner_shell_strength_electrons is None:
        scale = (1.0 - fjq_sum) / (1.0 - fj0_sum0)
        return fi0[None, :] * scale[:, None]

    inner_fraction = (
        np.broadcast_to(np.asarray(inner_shell_strength_electrons, float), fjq_sum.shape)
        / WATER_TOTAL_OSCILLATOR_STRENGTH
    )
    remaining = 1.0 - fjq_sum - inner_fraction
    if not np.all(np.isfinite(remaining)):
        raise ValueError("Finite-q oscillator-strength allocation is non-finite.")
    if np.any(remaining < -1.0e-12):
        raise ValueError("Excitation and K-shell strengths exceed the molecular f-sum.")
    fi0_sum = float(np.sum(fi0))
    if not np.isfinite(fi0_sum) or fi0_sum <= 0.0:
        raise ValueError("Outer-ionization oscillator strengths must have positive sum.")
    scale = np.maximum(remaining, 0.0) / fi0_sum
    return fi0[None, :] * scale[:, None]
...
def epsilon2_valence_Eq(
    ...
    fiq = _renorm_fi_q(
        ...
        fj0_sum0,
        fjq_sum,
        inner_shell_strength_electrons=inner_shell_strength_electrons,
    )                                             # (nq, n_ion)
...
def plot_model_vs_experiment_multiq(
    ...
    for q in qvals:
        ...
        if np.isclose(q, q_int):
            ...
            dat_path = ICE_DATA_XLSX_PATH.parent / f"ELFmodel_{tag}_ice_q{q_int}.dat"
            if dat_path.exists():
                try:
