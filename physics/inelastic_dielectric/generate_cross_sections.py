...
def _print_cli_help_and_exit():
    supported = ", ".join(PROJECTILE_LIBRARY)
    print(
        "Usage: ICE_TYPE=hexagonal python -m physics.inelastic_dielectric.generate_cross_sections [options]\n"
        "\n"
        "Options:\n"
...
def _born_kernel(s=None):
    return RPWBAKernel(
        mass_au=PROJECTILE_MASS_AU, nuclear_charge=PROJECTILE_CHARGE,
        kshell_model=KSHELL_MODEL, kshell_B_eV=KSHELL_B_EV,
        kshell_Zeff=KSHELL_ZEFF,
        elf_per_molecule=_ion_elf_per_molecule_factor(s) if s is not None else None,
        screening_ratio=_projectile_screening_ratio,
    )

def _elf_rolloff_factor(Ei):
    return _born_kernel(None)._elf_rolloff_factor(Ei)

def projectile_rest_energy_eV(projectile_mass_au=None):
    return _born_kernel(None).projectile_rest_energy_eV(projectile_mass_au)


def projectile_beta2(Tp_eV, projectile_mass_au=None):
    return _born_kernel(None).projectile_beta2(Tp_eV, projectile_mass_au)


def heavy_projectile_Emax(Tp_eV, projectile_mass_au=None):
    return _born_kernel(None).heavy_projectile_Emax(Tp_eV, projectile_mass_au)


def _projectile_energy_loss_upper_eV(Tp_eV):
    return _born_kernel(None)._projectile_energy_loss_upper_eV(Tp_eV)


def beta2_rel(Tj):
    return _born_kernel(None).beta2_rel(Tj)
...
def _q_bounds_scalar(Ei, Tj, projectile_mass_au=None):
    return _born_kernel(None)._q_bounds_scalar(Ei, Tj, projectile_mass_au)

def _q_bounds_scalar_rel(Ei, Tj):
    return _born_kernel(None)._q_bounds_scalar_rel(Ei, Tj)

def _projectile_relativistic_longitudinal_prefactor(Tj, s):
    return _born_kernel(s)._projectile_relativistic_longitudinal_prefactor(Tj, s)

def _rpwba_transverse_ratio(W_eV, q_au, beta2, epsilon1=None, epsilon2=None, use_density_effect=False):
    return _born_kernel(None)._rpwba_transverse_ratio(W_eV, q_au, beta2, epsilon1, epsilon2, use_density_effect)

# ----------------------------------------------------------------------
# Inner q-integral at fixed Ei for channels (excitation / ionization)
# ----------------------------------------------------------------------
def _integrate_channel_single_E(Ei, Tj, idx, channel_type, s, C, Nq=400, use_rel_bounds=False, include_kshell_sum_rule=None):
    return _born_kernel(s)._integrate_channel_single_E(Ei, Tj, idx, channel_type, s, C, Nq, use_rel_bounds, include_kshell_sum_rule)

def _integrate_channel_single_E_rpwba_components(Ei, Tj, idx, channel_type, s, C, Nq=400, use_density_effect=False, include_kshell_sum_rule=None):
    return _born_kernel(s)._integrate_channel_single_E_rpwba_components(Ei, Tj, idx, channel_type, s, C, Nq, use_density_effect, include_kshell_sum_rule)

def _integrate_channel_single_E_rel(Ei, Tj, idx, channel_type, s, C, Nq=400, include_kshell_sum_rule=None):
    return _born_kernel(s)._integrate_channel_single_E_rel(Ei, Tj, idx, channel_type, s, C, Nq, include_kshell_sum_rule)

def _integrate_channel_single_E_trans(Ei, Tj, idx, channel_type, s, C, Nq=400, use_density_effect=False, include_kshell_sum_rule=None):
    return _born_kernel(s)._integrate_channel_single_E_trans(Ei, Tj, idx, channel_type, s, C, Nq, use_density_effect, include_kshell_sum_rule)
...
def _kshell_threshold_eV(s):
    return _born_kernel(s)._kshell_threshold_eV(s)

def _kshell_old_optical_elf(Ei, s):
    return _born_kernel(s)._kshell_old_optical_elf(Ei, s)

def _kshell_hydrogenic_gos_elf(Ei, qvals, s):
    return _born_kernel(s)._kshell_hydrogenic_gos_elf(Ei, qvals, s)


def _ion_inner_shell_strength_electrons(qvals, include_kshell=None):
    return _born_kernel(None)._ion_inner_shell_strength_electrons(qvals, include_kshell)


def _ion_epsilon1_valence(E_arr, qvals, s, C, include_kshell=None):
    return _born_kernel(s)._ion_epsilon1_valence(E_arr, qvals, s, C, include_kshell)


def _ion_epsilon2_valence(E_arr, qvals, s, C, partitioned=True, include_kshell=None):
    return _born_kernel(s)._ion_epsilon2_valence(E_arr, qvals, s, C, partitioned, include_kshell)

# ----------------------------------------------------------------------
# Inner q-integral at fixed Ei for K-shell channel
# ----------------------------------------------------------------------
def _integrate_kshell_single_E(Ei, Tj, s, Nq=400, include_kshell=True, use_rel_bounds=False):
    return _born_kernel(s)._integrate_kshell_single_E(Ei, Tj, s, Nq, include_kshell, use_rel_bounds)

def _integrate_kshell_single_E_rpwba_components(Ei, Tj, s, C, Nq=400, include_kshell=True, use_density_effect=False):
    return _born_kernel(s)._integrate_kshell_single_E_rpwba_components(Ei, Tj, s, C, Nq, include_kshell, use_density_effect)

def _integrate_kshell_single_E_rel(Ei, Tj, s, C, Nq=400, include_kshell=True):
    return _born_kernel(s)._integrate_kshell_single_E_rel(Ei, Tj, s, C, Nq, include_kshell)
...
def save_cross_section_corrections_npz(
    ...
    np_save_args.update(_barkas_generation_metadata(include_barkas_dcs))
    ...
    if dcs_data is not None:
        ...
        if diag is not None:
            np_save_args.update(diag.barkas_model_metadata)
            np_save_args["barkas_quadrature_error_m2_per_eV"] = np.asarray(diag.barkas_quadrature_error_m2_per_eV, float)
...
def _npz_matches_params(
    ...
    if stored_barkas != bool(include_barkas_dcs):
        return False
    for key, expected in _barkas_generation_metadata(include_barkas_dcs).items():
        if key not in npz_data or np.asarray(npz_data[key]).item() != expected:
            return False
...
def _dcs_data_from_npz(npz_data):
    ...
    return {
        ...
        "barkas_model_metadata": {
            key: np.asarray(npz_data[key]).item()
            for key in _barkas_generation_metadata(include_barkas_dcs)
            if key in npz_data
        },
        "born_reference_charge": str(born_reference_charge),
...
def _export_to_custom_geant4(paths):
    """Copy only when the destination is explicitly requested with --geant4-data-dir."""
    destination = _argv_value(("--geant4-data-dir",), default=None)
    if destination is None:
        return
    dna_dir = Path(destination).expanduser().resolve()
    if not dna_dir.is_dir():
        raise FileNotFoundError(f"Geant4 data destination does not exist: {dna_dir}")
    for path in paths:
        if path is not None:
            shutil.copy2(path, dna_dir / path.name)
    print(f"Exported tables to {dna_dir}")
