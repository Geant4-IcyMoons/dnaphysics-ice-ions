...
from physics.elastic.bca.config import DEFAULT_PROJECTILES, DEFAULT_WORKERS  # noqa: E402
from physics.elastic.bca.convergence import (  # noqa: E402
    ...
    convergence_report,
    relative_standard_error_report,
...
from physics.elastic.bca.runtime import AdaptiveKernelTable  # noqa: E402
from physics.elastic.bca.structure import IceStructure, load_ice_structure  # noqa: E402
from physics.elastic.bca.trajectory import (  # noqa: E402
    HardTrajectoryResult,
    PeriodicHardCollisionTransport,
)

from physics.elastic.zbl.backend import (  # noqa: E402
    FullZBLKernel,
    SoftZBLKernel,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--statistical-relative-tolerance", type=float,
                        help="Estimated whole-history scalar SE target; excludes CDF qualification")
    parser.add_argument(
        "--statistical-confidence",
        type=float,
        default=DEFAULT_STATISTICAL_CONFIDENCE,
        help=(
            "Family-wise confidence across every scalar/CDF target, look, "
            "and declared campaign case "
            "(default: 0.95)."
        ),
    )
    parser.add_argument(
        "--simultaneous-case-count",
        type=int,
        default=1,
        help=(
            "Number of cases sharing the family-wise confidence budget. "
            "A standalone calculation uses one; campaign controllers must "
            "pass their predeclared maximum case count."
        ),
    )
    parser.add_argument(
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    relative = args.statistical_relative_tolerance
    if relative is not None and (not math.isfinite(relative) or not 0 < relative < 1):
        raise ValueError("Relative SE tolerance must lie in (0,1)")
    if relative is not None and (args.statistical_absolute_tolerances_json is not None or args.trajectory_cdf_tolerance is not None):
        raise ValueError("Choose scalar relative SE or the absolute/CDF contract, not both")
    if args.statistical_absolute_tolerances_json is None:
        if args.trajectories is None and relative is None:
            raise ValueError(
    ...
    if args.simultaneous_case_count < 1:
        raise ValueError("--simultaneous-case-count must be positive.")
...
def _assess_statistics(args, statistics, **kwargs):
    if args.statistical_relative_tolerance is not None:
        return relative_standard_error_report(statistics, args.statistical_relative_tolerance,
                                               kwargs["look_index"], kwargs["scheduled_look_count"])
    return convergence_report(statistics, **kwargs)
...
def _write_manifest(
    ...
    raw_statistical_report = _assess_statistics(args,
        raw_statistics,
        ...
        simultaneous_case_count=args.simultaneous_case_count,
    )
    ...
    manifest = {
        ...
        "configuration": {
            ...
            "statistical_relative_tolerance": args.statistical_relative_tolerance,
...
def main() -> int:
    ...
    schedule = (
        (progressive_schedule if args.statistical_relative_tolerance is not None else doubling_schedule)(
    ...
    try:
        for look_index, target in enumerate(schedule, start=1):
            ...
            if absolute_tolerances is None and args.statistical_relative_tolerance is None:
            ...
            last_report = _assess_statistics(args,
                statistics,
            ...
            maximum_width = last_report.get("maximum_normalized_standard_error", last_report["maximum_normalized_confidence_width"])
            width_text = (
            ...
            print(
                ...
                f"N={completed:,}, maximum normalized precision statistic="
                f"{width_text}; "
                + (
                    "CDF half-width="
