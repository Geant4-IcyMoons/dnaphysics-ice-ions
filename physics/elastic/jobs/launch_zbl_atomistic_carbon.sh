...
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/physics/elastic/zbl/runs/carbon_scalar}"
MAX_ARRAY_SIZE="${MAX_ARRAY_SIZE:-50}"

cd "${REPO_ROOT}"
"${PYTHON_BIN}" -m \
  physics.elastic.zbl.run_campaign \
  ...
  --transfer-cutoffs-ev 1e-4 1e-5 1e-6 --tolerance 0.05 --confidence 0.95
