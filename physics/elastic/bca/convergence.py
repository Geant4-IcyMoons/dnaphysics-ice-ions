...
TRAJECTORY_CDF_COUNT = 2
TOTAL_ACCEPTANCE_TARGETS = len(OBSERVABLES) + TRAJECTORY_CDF_COUNT
...
class RatioStatistics:
    ...
    def interval(self, critical_value: float) -> dict[str, float | bool | None]:
        ...
        return {
            ...
            "confidence_lower": max(0.0, estimate - half_width),
            "confidence_upper": estimate + half_width,
            "finite": finite,
        }
...
def simultaneous_critical_value(
    ...
    confidence: float,
    acceptance_target_count: int,
    scheduled_look_count: int,
    simultaneous_case_count: int = 1,
) -> tuple[float, float]:
    """Student-t critical value under one complete Bonferroni budget."""
    ...
    if (
        acceptance_target_count < 1
        or scheduled_look_count < 1
        or simultaneous_case_count < 1
    ):
        raise ValueError(
            "Acceptance-target, scheduled-look, and case counts must be positive."
        )
    individual_alpha = (1.0 - confidence) / (
        acceptance_target_count * scheduled_look_count * simultaneous_case_count
    )
    critical = float(
...
def simultaneous_dkw_half_width(
    ...
    """Distribution-free CDF band under the complete Bonferroni budget."""
...
def convergence_report(
    ...
    *,
    ...
    critical, individual_confidence = simultaneous_critical_value(
        trajectory_count,
        confidence,
        TOTAL_ACCEPTANCE_TARGETS,
        scheduled_look_count,
        simultaneous_case_count,
    )
    ...
    return {
        ...
        "simultaneous_case_count": simultaneous_case_count,
        "acceptance_target_count": TOTAL_ACCEPTANCE_TARGETS,
        ...
        "method": (
            ...
            "one Bonferroni correction across scalar observables, trajectory "
            "CDFs, scheduled looks, and declared campaign cases"
        ),
        "references": (
...
def relative_standard_error_report(statistics, tolerance, look_index, scheduled_look_count):
    """Estimated whole-history ratio SE; no simultaneous or stopping guarantee."""
    if set(statistics) != set(OBSERVABLES) or not 0 < tolerance < 1:
        raise ValueError("Invalid scalar precision contract")
    counts = {v.count for v in statistics.values()}
    if len(counts) != 1 or not 1 <= look_index <= scheduled_look_count:
        raise ValueError("Inconsistent histories or look")
    reports = {}
    for name, stat in statistics.items():
        value = stat.interval(1.)
        estimate, se = value["estimate"], value["standard_error"]
        relative = se/abs(estimate) if estimate is not None and estimate > 0 and se is not None else None
        reports[name] = {"estimate": estimate, "standard_error": se,
            "relative_standard_error": relative, "unit": OBSERVABLE_UNITS[name],
            "passes": relative is not None and 0 < relative <= tolerance,
            "zero_variance_requires_review": se == 0.,
            "confidence_half_width": None, "relative_confidence_half_width": None}
    values = [r["relative_standard_error"]/tolerance for r in reports.values() if r["relative_standard_error"] is not None]
    return {"converged": all(r["passes"] for r in reports.values()), "observables": reports,
        "trajectory_count": counts.pop(), "look_index": look_index, "scheduled_look_count": scheduled_look_count,
        "absolute_tolerances": None, "relative_tolerance": tolerance,
        "precision_mode": "relative_standard_error", "distribution_qualified": False,
        "maximum_normalized_confidence_width": None,
        "maximum_normalized_standard_error": max(values) if values else None,
        "method": "Whole-history weighted ratio delta-method SE; no simultaneous or optional-stopping guarantee"}


def progressive_schedule(minimum, maximum, batch_size):
    # Validate the same computational limits as the legacy schedule.
    first = doubling_schedule(minimum, maximum, batch_size)[0]
    schedule = [first]
    while schedule[-1] < maximum:
        schedule.append(min(maximum, math.ceil(1.2*schedule[-1]/batch_size)*batch_size))
    return tuple(schedule)
