...
SIMULATOR = (
    REPOSITORY_ROOT
    / "physics/elastic/simulate.py"
)
IMPLEMENTATION_SOURCES = (
    SIMULATOR,
    REPOSITORY_ROOT / "physics/elastic/bca/convergence.py",
    REPOSITORY_ROOT
    / "physics/elastic/bca/trajectory.py",
    REPOSITORY_ROOT
    / "physics/elastic/bca/scattering.py",
    REPOSITORY_ROOT
    / "physics/elastic/nlh/coefficients.csv",
    Path(__file__).resolve().parent / "backend.py",
    Path(__file__).resolve().parent / "kernel.py",
    Path(__file__).resolve(),
)
SCALAR_RELATIVE_TOLERANCE = 0.05
NUMERICAL_RELATIVE_TOLERANCE = 0.05
...
def prepare_campaign(
    ...
    *,
    ...
    configuration = {
        ...
        "trajectory_cdf_absolute_tolerance": None,
        "precision_mode": "relative_standard_error",
        "sampling_growth": 0.2,
...
def _campaign(path: Path) -> dict[str, Any]:
    ...
    signature = hashlib.sha256(json.dumps(value["configuration"], sort_keys=True, separators=(",", ":")).encode()).hexdigest()
    if signature != value["configuration_signature"]:
        raise RuntimeError("Campaign signature mismatch")
    return value


def check_scalar_report(report, configuration):
    if (not report.get("converged") or report.get("precision_mode") != "relative_standard_error"
            or report.get("relative_tolerance") != configuration["statistical_relative_tolerance"]):
        raise RuntimeError("Case precision contract differs from campaign")
    from physics.elastic.bca.convergence import OBSERVABLES
    if set(report["observables"]) != set(OBSERVABLES):
        raise RuntimeError("Missing scalar observables")
    for value in report["observables"].values():
        mean, se = value["estimate"], value["standard_error"]
        if (mean is None or not math.isfinite(mean) or mean <= 0 or se is None
                or not math.isfinite(se) or not 0 < se/mean <= report["relative_tolerance"]):
            raise RuntimeError("Case scalar precision failed on reassessment")
...
def run_case(
    ...
    if manifest_path.is_file():
        ...
        if isinstance(report, dict) and report.get("converged") is True:
            check_scalar_report(report, configuration)
    ...
    if manifest["statistical_convergence"].get("converged") is not True:
        raise RuntimeError(f"ZBL case did not satisfy the scalar precision gate: {manifest_path}")
    check_scalar_report(manifest["statistical_convergence"], configuration)
...
def cutoff_comparison(group, reports):
    """Diagnostic adjacent-cutoff convergence for stopping and transport.

    Collision counts depend on the transport cutoff. They are not required to
    approach a cutoff-independent total as arbitrarily weak events are added.
    """
    comparisons = []
    for (fine, a), (coarse, b) in zip(sorted(reports), sorted(reports)[1:]):
        checks = {}
        for name in ("hard_nuclear_stopping_ev_per_angstrom", "hard_transport_rate_per_angstrom"):
            x, y = a["observables"][name], b["observables"][name]
            scale = min(x["estimate"], y["estimate"])
            delta = abs(x["estimate"]-y["estimate"])
            se = math.hypot(x["standard_error"], y["standard_error"])
            checks[name] = {"relative_change": delta/scale if scale > 0 else None,
                "relative_comparison_se": se/scale if scale > 0 else None,
                "passes": scale > 0 and delta <= NUMERICAL_RELATIVE_TOLERANCE*scale and se <= SCALAR_RELATIVE_TOLERANCE*scale}
        comparisons.append({"fine_cutoff_ev": fine, "coarse_cutoff_ev": coarse, "observables": checks})
    return {"case_group": list(group), "comparisons": comparisons,
        "scope": "Independent-cohort estimated SE and observed change at tested cutoffs; not a handoff certificate"}
...
def reduce_campaign(campaign_path: Path) -> tuple[Path, Path, Path]:
    ...
    for case in tqdm(campaign["cases"], desc="Reducing ZBL cases", unit="case"):
        ...
        cdf = report.get("trajectory_cdf_convergence", {"absolute_confidence_half_width": None})
        check_scalar_report(report, campaign["configuration"])
        group = (case["phase_id"], case["structure_index"], case["orientation"], case["energy_ev"])
        cutoff_groups.setdefault(group, []).append((case["minimum_transfer_ev"], report))
        cross_sections.append(
            {
                ...
                "sigma_standard_error_angstrom2": rate["standard_error"] / density,
                "relative_standard_error": rate["relative_standard_error"],
    ...
    _atomic_json(
        final_path,
        {
            ...
            "numerical_status": "all_cases_meet_scalar_relative_standard_error_target",
            "relative_standard_error_target": campaign["configuration"]["statistical_relative_tolerance"],
            "distribution_qualified": False,
            "handoff_qualified": False,
            "soft_cutoff_qualified": False,
            "cutoff_comparisons": [cutoff_comparison(group, reports) for group, reports in sorted(cutoff_groups.items())],
            "physics_status": "validation_pending",
...
def _parser() -> argparse.ArgumentParser:
    ...
    prepare.add_argument("--tolerance", type=float, default=SCALAR_RELATIVE_TOLERANCE)
