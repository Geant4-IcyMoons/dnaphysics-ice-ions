...
- Saves figures in all modes; interactive display is controlled by --show-mode.
"""
...
from physics_ice.root_utils import resolve_root_paths, resolve_first_root
...
DEFLECTION_BIN_COUNT = 36000  # 0.005 deg bins over [0, 180]
DEFLECTION_RANGE_DEG = (0.0, 180.0)
PLOT_SHOW_MODE = "all"  # "all" | "each" | "none"
...
def _finalize_figure(fig, out_path: str) -> str:
    """Save a figure and handle display mode consistently."""
    outpath = _resolve_output(out_path)
    fig.savefig(outpath, bbox_inches="tight")
    print(f"Wrote {outpath}")
    if PLOT_SHOW_MODE == "none":
        plt.close(fig)
    elif PLOT_SHOW_MODE == "each":
        plt.show()
        plt.close(fig)
    return outpath


def _show_pending_figures_if_needed() -> None:
    """Show all pending figures at script end when using grouped display mode."""
    if PLOT_SHOW_MODE != "all":
        return
...
def _particle_name_map() -> dict[int, str]:
    return {
        0: "gamma",
        1: "electron",
        2: "proton",
...
def _particle_label_from_flag(flag: int) -> str:
    return _particle_name_map().get(int(flag), f"flag{int(flag)}")

def _summary_category_styles() -> dict[str, tuple[list[int], str]]:
    return {
        "Solv": ([10], "#9467bd"),
        "Elastic": ([11, 21, 31, 41, 51, 61, 110, 210, 410, 510, 710, 120, 220, 420, 520, 720], "#d62728"),
        "Excit": ([12, 15, 22, 32, 42, 52, 62], "#2ca02c"),
        "Ionis": ([13, 23, 33, 43, 53, 63, 73, 130, 230, 430, 530, 730], "#1f77b4"),
        "Attach": ([14], "#8c564b"),
        "Charge": ([24, 34, 44, 54, 64, 74, 35, 55, 65, 740], "#ff7f0e"),
    }

def _summary_hist_process_ids() -> list[int]:
    out: set[int] = set()
    for ids, _ in _summary_category_styles().values():
        out.update(int(x) for x in ids)
    return sorted(out)

def _process_family(pcode: int) -> str:
    p = int(pcode)
    if p in (12, 15, 22, 32, 42, 52, 62):
        return "excitation"
    if p in (13, 23, 33, 43, 53, 63, 73):
        return "ionisation"
    if p in (11, 21, 31, 41, 51, 61, 110, 210, 410, 510, 710, 120, 220, 420, 520, 720):
        return "elastic"
    if p in (14,):
        return "attachment"
    return "other"

def _infer_primary_flag(
    flag_particle: np.ndarray,
    parent_id: np.ndarray | None = None,
    track_id: np.ndarray | None = None,
) -> int:
    fp = np.asarray(flag_particle, dtype=int)
    if fp.size == 0:
        return 1
    mask = np.ones(fp.shape, dtype=bool)
    if parent_id is not None:
        par = np.asarray(parent_id, dtype=int)
        if par.shape == fp.shape:
            mask &= (par == 0)
    if track_id is not None:
        trk = np.asarray(track_id, dtype=int)
        if trk.shape == fp.shape:
            mask &= (trk == 1)
    candidates = fp[mask] if np.any(mask) else fp
    uniq, counts = np.unique(candidates, return_counts=True)
    if uniq.size == 0:
        return 1
    return int(uniq[np.argmax(counts)])

def _compute_deflection_xlim(theta_values_deg: np.ndarray) -> tuple[float, str]:
    vals = np.asarray(theta_values_deg, dtype=float)
    vals = vals[np.isfinite(vals) & (vals >= 0.0)]
    if vals.size == 0:
        return 180.0, "0-180 deg"
    if vals.size >= 10:
        q99 = float(np.percentile(vals, 99.0))
    else:
        q99 = float(np.max(vals))
    q99 = max(q99, 0.001)
    # If almost all values are very forward-peaked, zoom in to expose shape.
    if q99 < 10.0:
        xlim = _deflection_xlim_from_quantile(q99)
        return xlim, f"zoom 0-{xlim:.2f} deg (99th pct)"
    return 180.0, "0-180 deg"

def _compute_deflection_xlim_from_hist(theta_centers_deg: np.ndarray, counts: np.ndarray) -> tuple[float, str]:
    centers = np.asarray(theta_centers_deg, dtype=float)
    hist = np.asarray(counts, dtype=float)
    valid = np.isfinite(centers) & np.isfinite(hist) & (hist > 0.0)
    if not np.any(valid):
        return 180.0, "0-180 deg"
    ...
    if total <= 0.0:
        return 180.0, "0-180 deg"
    q99 = float(centers[np.searchsorted(cdf / total, 0.99, side="left")])
    q99 = max(q99, 0.001)
    if q99 < 10.0:
        xlim = _deflection_xlim_from_quantile(q99)
        return xlim, f"zoom 0-{xlim:.2f} deg (99th pct)"
...
def _deflection_xlim_from_quantile(q99_deg: float) -> float:
    target = float(max(0.001, q99_deg) * 3.0)
    # Prefer compact "nice" bounds so ultra-forward proton scatters remain visible.
    for upper in (0.05, 0.10, 0.20, 0.50, 1.0, 2.0, 5.0, 10.0, 20.0):
        if target <= upper:
            return float(upper)
    return 20.0

def _compute_initial_angle_xlim(theta_all_deg: np.ndarray, theta_incomplete_deg: np.ndarray) -> tuple[float, str]:
    vals = np.concatenate([np.asarray(theta_all_deg, dtype=float), np.asarray(theta_incomplete_deg, dtype=float)])
    vals = vals[np.isfinite(vals) & (vals >= 0.0)]
    if vals.size == 0:
        return 90.0, "0-90 deg"
    q99 = float(np.percentile(vals, 99.0)) if vals.size >= 10 else float(np.max(vals))
    q99 = max(q99, 0.001)
    if q99 < 20.0:
        xlim = _deflection_xlim_from_quantile(q99)
        return xlim, f"zoom 0-{xlim:.2f} deg (99th pct)"
...
def _initial_angle_bins(xlim_deg: float) -> np.ndarray:
    if xlim_deg <= 0.2:
        n_bins = 120
    elif xlim_deg <= 2.0:
        n_bins = 100
        ...
        n_bins = 90
    return np.linspace(0.0, float(xlim_deg), int(n_bins) + 1)
...
def _count_xs_figures_for_stream(meta: dict, proc_list: list[int]) -> int:
    total = 0
    for pcode in proc_list:
        models = [m for m in meta.get("models_by_proc", {}).get(int(pcode), set()) if m]
        total += max(1, len(models))
    return total


def _count_xs_figures_for_arrays(arrs, proc_list: list[int]) -> int:
    if arrs is None or "flagProcess" not in arrs:
        return 0
    flag_proc = np.asarray(arrs["flagProcess"], dtype=float)
    model_names = _decode_to_str_array(arrs.get("modelName"))
    total = 0
    for pcode in proc_list:
        mask = (flag_proc == float(pcode))
        if not np.any(mask):
            continue
        if model_names is None:
            total += 1
            continue
        uniq_models = [m for m in np.unique(model_names[mask]) if m]
        total += max(1, len(uniq_models))
    return total


def _count_deflection_figures_for_stream(meta: dict) -> int:
    return len(meta.get("ke_min", {}))


def _count_deflection_figures_for_arrays(arrs) -> int:
    if arrs is None or "flagProcess" not in arrs:
        return 0
    flag_process = np.asarray(arrs["flagProcess"], dtype=int)
    model_names = _decode_to_str_array(arrs.get("modelName"))
    if model_names is None:
        return int(np.unique(flag_process).size)
    seen: set[tuple[int, str]] = set()
    for p, m in zip(flag_process, model_names):
        seen.add((int(p), m if m else ""))
    return len(seen)


def _count_vib_figures_for_stream(meta: dict) -> int:
    if int(15) not in meta.get("proc_counts", {}):
        return 0
    models = [m for m in meta.get("models_by_proc", {}).get(15, set()) if m]
    return max(1, len(models))


def _count_vib_figures_for_arrays(arrs) -> int:
    if arrs is None or "flagProcess" not in arrs:
        return 0
    ...
    if model_names is None:
        return 1
    models = [m for m in np.unique(model_names[m_vib]) if m]
    return max(1, len(models))


def _print_plot_plan(xs_count: int, deflection_count: int, vib_count: int,
                     summary_count: int = 1, initial_count: int = 1) -> None:
    total = int(xs_count) + int(deflection_count) + int(vib_count) + int(summary_count) + int(initial_count)
    print(
        "Plot plan: "
        f"{xs_count} cross-section, "
        f"{deflection_count} deflection, "
        f"{vib_count} vib, "
        f"{summary_count} summary, "
        f"{initial_count} initial-angle "
        f"=> total {total} figure(s)"
    )
    if total >= 25:
        print("Warning: large figure count. Use --process or --processes to narrow the run, or --no-show to save without opening windows.")
...
def _iter_root_arrays(path: str, tree_name: str, columns: list[str], step_size: int):
    ...
    with uproot.open(paths[0]) as f:
        if tree_name not in f:
            raise RuntimeError(f"Tree '{tree_name}' not found in {paths[0]}")
        t = f[tree_name]
        columns = [c for c in columns if c in t.keys()]
    tree_spec = [f"{p}:{tree_name}" for p in paths]
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", message="overflow encountered in scalar add", category=RuntimeWarning)
...
def extract_initial_angle_distributions(
    ...
    for path in paths:
        with uproot.open(path) as rootf:
            ...
            step_required = ["eventID", "parentID", "x", "y", "z"]
...
def plot_initial_angle_diagnostics(
    ...
    xlim_max, xlim_label = _compute_initial_angle_xlim(theta_all, theta_incomplete)
    bins = _initial_angle_bins(xlim_max)

    ax_all.hist(theta_all, bins=bins, histtype="stepfilled", alpha=0.85, color="lightgray")
    ax_all.set_xlim(0.0, xlim_max)
    ...
    ax_incomplete.set_xlim(0.0, xlim_max)
...
def _stream_collect_meta(path: str, tree_name: str, step_size: int) -> dict:
    ...
    for chunk in _iter_root_arrays(path, tree_name, columns, step_size):
        ...
        # Primary-particle inference and per-flag kmax for summary
        if "flagParticle" in chunk:
            fp = np.asarray(chunk["flagParticle"], dtype=int)
            m_primary = np.ones(fp.shape, dtype=bool)
            if "parentID" in chunk:
                m_primary &= (np.asarray(chunk["parentID"], dtype=int) == 0)
            if "trackID" in chunk:
                m_primary &= (np.asarray(chunk["trackID"], dtype=int) == 1)

            fp_primary = fp[m_primary] if np.any(m_primary) else fp
            if fp_primary.size:
                uniq_fp, cnt_fp = np.unique(fp_primary, return_counts=True)
                for flag, cnt in zip(uniq_fp.tolist(), cnt_fp.tolist()):
                    meta["primary_flag_counts"][int(flag)] = (
                        meta["primary_flag_counts"].get(int(flag), 0) + int(cnt)
                    )

            for flag in np.unique(fp):
                m_flag = (fp == int(flag))
                if np.any(m_primary):
                    m_flag &= m_primary
                if not np.any(m_flag):
                    continue
                ke_flag = kinE[m_flag]
                ke_flag = ke_flag[np.isfinite(ke_flag) & (ke_flag > 0)]
                if ke_flag.size:
                    meta["kmax_by_flag"][int(flag)] = max(
                        meta["kmax_by_flag"].get(int(flag), 0.0),
                        float(np.max(ke_flag)),
                    )
    ...
    if meta["primary_flag_counts"]:
        meta["primary_flag"] = int(
            max(meta["primary_flag_counts"].items(), key=lambda kv: kv[1])[0]
        )
    elif meta["kmax_by_flag"]:
        meta["primary_flag"] = int(
            max(meta["kmax_by_flag"].items(), key=lambda kv: kv[1])[0]
        )
    return meta
...
def _init_stream_acc(meta: dict, nH2O_cm3: float) -> dict:
    ...
    for key, e_min in meta.get("ke_min", {}).items():
        ...
        family = _process_family(int(key[0]))
        has_chan_micro = bool(meta.get("has_chan_micro", {}).get(key, False))
        use_counts = family in ("excitation", "ionisation") and (not has_chan_micro)
        acc["xs"][key] = {
            "bins": bins,
            "centers": centers,
            "is_log": is_log,
        ...
        acc["deflection"][key] = np.zeros(DEFLECTION_BIN_COUNT, dtype=int)
    ...
    summary["x_hist_by_proc"] = {
        pid: np.zeros(100, dtype=int) for pid in _summary_hist_process_ids()
    }
    primary_flag = int(meta.get("primary_flag", 1))
    summary["primary_flag"] = primary_flag
    summary["primary_label"] = _particle_label_from_flag(primary_flag)
    kmax = float(meta.get("kmax_by_flag", {}).get(primary_flag, 0.0))
    if (not np.isfinite(kmax) or kmax <= 0) and meta.get("kmax_by_flag"):
        kmax = float(max(meta["kmax_by_flag"].values()))
...
def _stream_accumulate(path: str, tree_name: str, step_size: int, nH2O_cm3: float, meta: dict, acc: dict) -> None:
    ...
    theta_bins = np.linspace(
        DEFLECTION_RANGE_DEG[0],
        DEFLECTION_RANGE_DEG[1],
        DEFLECTION_BIN_COUNT + 1,
    )
    ...
    for chunk in _iter_root_arrays(path, tree_name, columns, step_size):
        ...
        if "flagParticle" in chunk:
            fp = np.asarray(chunk["flagParticle"], dtype=int)
            primary_flag = int(summary.get("primary_flag", 1))
            m_primary = (fp == primary_flag)
            if np.any(m_primary):
                h, _ = np.histogram(kinE[m_primary], bins=summary["k_bins"])
                summary["k_hist"] += h
                # Reservoir-ish sample for scatter
                if "x" in chunk and "y" in chunk and "z" in chunk:
                    x = np.asarray(chunk["x"], dtype=float)[m_primary]
                    y = np.asarray(chunk["y"], dtype=float)[m_primary]
                    z = np.asarray(chunk["z"], dtype=float)[m_primary]
                    if x.size:
...
def plot_cross_sections_for_process(arrs, pcode: int, ncols: int, scale: float,
    ...
    fig, axes = plt.subplots(
        ...
        squeeze=False, sharex=False, sharey=False, constrained_layout=False
    )
    ...
    use_counts = (chan_idx is not None and np.any(chan_idx >= 0)) and (not has_chan_micro)
    series_by_ch = None
    total_series = None
...
def plot_summary_stream(acc: dict, out_path: str, fontsize: float):
    ...
    if proc_counts:
        ...
        if np.any(counts > 0):
            ax1.set_yscale("log")
    ax1.set_xlabel("flagProcess"); ax1.set_ylabel("Counts")
    ...
    for lab, (ids, col) in _summary_category_styles().items():
        h_total = np.zeros_like(centers, dtype=float)
        for pid in ids:
            h_pid = summary["x_hist_by_proc"].get(int(pid))
            if h_pid is not None:
                h_total += h_pid
        if not np.any(h_total > 0):
            continue
        has_positive_panel3 = True
        ax3.plot(centers, h_total, label=lab, color=col)
    ax3.set_xlabel("x (nm)"); ax3.set_ylabel("Counts")
    if has_positive_panel3:
        ax3.set_yscale("log")
...
def plot_deflection_angles_all_stream(acc: dict, out_path: str, fontsize: float = FONTSIZE):
    ...
    for key, hist in acc["deflection"].items():
        ...
        ax.hist(theta_centers, bins=theta_bins, weights=hist, histtype="step", linewidth=1.8, color="#1f5fae")
        xlim_max, xlim_label = _compute_deflection_xlim_from_hist(theta_centers, hist)
...
def plot_summary(arrs, out_path: str, fontsize: float):
    ...
    if fp is None or fl_p is None or kinE is None or x_nm is None or y_nm is None or z_nm is None:
        return None
    fp_i = np.asarray(fp, dtype=int)
    primary_flag = _infer_primary_flag(
        fp_i,
        arrs.get("parentID"),
        arrs.get("trackID"),
    )
    ...
    has_positive_panel3 = False
    for lab, (ids, col) in _summary_category_styles().items():
        m = np.isin(fl_pi, np.asarray(ids, dtype=int))
...
def plot_deflection_angles_all(arrs, out_path: str, fontsize: float = FONTSIZE):
    ...
    for proc_code, model_name in unique_keys:
        ...
        if thetas.size == 0:
            plt.close(fig)
            continue
        zero_mask = np.isclose(thetas, 0.0, rtol=0.0, atol=1e-12)
        zero_frac = float(np.mean(zero_mask)) if thetas.size else 0.0
        hist_vals, edges = np.histogram(
            thetas,
            bins=DEFLECTION_BIN_COUNT,
            range=DEFLECTION_RANGE_DEG,
        )
        ...
        xlim_max, xlim_label = _compute_deflection_xlim(thetas)
        ax.set_xlim(0, xlim_max)
        ax.set_xlabel(r"$\theta$ (deg)")
        ax.set_ylabel("Counts")
        title = model_name if model_name else f"proc {int(proc_code)}"
        ...
        if zero_frac >= 0.9999:
            ax.text(
                0.98,
                0.95,
                "All sampled deflections are 0 deg",
            ...
            ax.text(
                0.98,
                0.95,
                f"{100.0 * zero_frac:.1f}% of steps at 0 deg",
                transform=ax.transAxes,
                ha="right",
                va="top",
                fontsize=max(8, int(fontsize * 0.65)),
...
def main():
    ...
    ap.add_argument(
        ...
        default="all",
        help="Figure display mode: all=show once at end (tabs/windows together), each=show each figure sequentially, none=save only",
    )
    ap.add_argument("--no-show", action="store_true", help="Alias for --show-mode none")
    args = ap.parse_args()

    if args.no_show:
        args.show_mode = "none"
    PLOT_SHOW_MODE = str(args.show_mode)
    ...
    if args.stream:
        ...
        if args.processes is not None and str(args.processes).strip().lower() == "all":
        ...
        _print_plot_plan(
            xs_count=_count_xs_figures_for_stream(meta, proc_list),
            deflection_count=_count_deflection_figures_for_stream(meta),
            vib_count=_count_vib_figures_for_stream(meta),
        )
        ...
        _show_pending_figures_if_needed()
        return
    ...
    # Determine processes to plot
    if arrs is not None and args.processes is not None and str(args.processes).strip().lower() == 'all':
    ...
    _print_plot_plan(
        xs_count=_count_xs_figures_for_arrays(arrs, proc_list),
        deflection_count=_count_deflection_figures_for_arrays(arrs),
        vib_count=_count_vib_figures_for_arrays(arrs),
    )
