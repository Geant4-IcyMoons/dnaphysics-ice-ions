...
import scipy
import projectile_form_factors as ff
import generate_ice_cross_sections_ion as gen
from constants import DIAGNOSTIC_PLOTS_DIR, FONT_COURIER
...
def make_plot(output, states):
    ...
    fig.savefig(output/"projectile_form_factors.png", dpi=240)
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=DIAGNOSTIC_PLOTS_DIR/"projectile_form_factors")
    ...
    report = dict(model=ff.VERSION, atomic_data_sha256=hashlib.sha256(ff.DATA_PATH.read_bytes()).hexdigest(),
                  source_sha256={name: hashlib.sha256((PHYSICS/name).read_bytes()).hexdigest()
                                 for name in ("projectile_form_factors.py", "generate_ice_cross_sections_ion.py",
                                              "emfietzoglou_model_finite_q.py", "generate_projectile_atomic_data.py")},
