...
The physical-time equations are autonomous. DOP853 therefore evaluates each
state update from a local time origin and accumulates elapsed time separately
with compensated summation. There is no absolute `10*machine_epsilon` floor
on the local step: a finite-periapsis Coulomb passage can require a smaller
time step while `h*velocity` remains well resolved in the state. The solver
rejects only a non-positive or non-finite step. Deterministic regressions retain
the two production trajectories that exposed the former, contradictory floor;
both now complete with independently checked relative energy drift below
`1e-5`.
...
adaptive manifest, but withholds the final tables until every nested-grid task
satisfies a 0.25% limit on each numerical axis, their conservative sum is at
most 0.5%, and every active cross section has a two-sided asymptotic 95%
Monte Carlo confidence half-width at most 0.5%. Reuse the same 84-way layout
in two dependency-ordered compute stages:

```bash
# After the base merge:
qsub -J 0-41 \
  -v START_SEPARATION_AU,TARGET_BMAX_AU,LOSS_BMAX_AU,SHARD_COUNT=84,ADAPTIVE_BASE_CURVES_ONLY=1 \
  pbs/generate_carbon_charge_exchange_ctmc.pbs
qsub -J 0-41 \
  -v START_SEPARATION_AU,TARGET_BMAX_AU,LOSS_BMAX_AU,SHARD_COUNT=84,SHARD_OFFSET=42,ADAPTIVE_BASE_CURVES_ONLY=1 \
  pbs/generate_carbon_charge_exchange_ctmc.pbs

# After every base-curve element completes, repeat both arrays with
# ADAPTIVE_INTERVALS_ONLY=1 and without ADAPTIVE_BASE_CURVES_ONLY.

# After every adaptive interval element completes:
qsub -l select=1:ncpus=1:mem=4gb \
  -v START_SEPARATION_AU,TARGET_BMAX_AU,LOSS_BMAX_AU,SHARD_COUNT=84,MERGE_ADAPTIVE_ONLY=1 \
  pbs/generate_carbon_charge_exchange_ctmc.pbs
```

The first stage owns each unique `(base energy, charge)` curve exactly once.
The second owns one charge state and one original energy interval, reuses both
shared endpoints, and calculates logarithmic energy midpoints only as required
by convergence. For carbon, this removes 1.29012 billion duplicated endpoint
trajectories from the mandatory first refinement level. If the confidence-width
gate fails, the same coordinate-keyed random stream continues with twice as
many trajectories;
...
IEVM/IPM estimator. Task checkpoints are independent of the PBS shard count,
so refinement can resume under a different layout without recomputing
completed work. Use `--no-adaptive-refinement` only for an explicitly labelled
