...
automatically. For a multi-shard run, use the five restart-safe stages:
...
# After all unique base-curve shards complete:
bash pbs/launch_charge_exchange_ctmc_example.sh refine-intervals carbon 84
# After all adaptive interval shards complete:
bash pbs/launch_charge_exchange_ctmc_example.sh merge-refined carbon 84
```

The first adaptive stage partitions the 41 base energies and seven carbon
charges into 287 unique impact-curve tasks. The second partitions the 40
original energy intervals and seven charges into 280 families. Interval
families load the two completed shared endpoint curves and calculate only new
logarithmic midpoints; adjacent intervals never recompute their common
endpoint. Every task has an atomic trajectory checkpoint and completed result,
so cancellation loses only uncommitted in-flight chunks. The files remain
usable if the PBS shard count changes on resume. Use
`--no-adaptive-refinement` only when intentionally producing a separately
labelled fixed-grid reference.

## Carbon paper benchmark and release checks

After the final adaptive merge, generate the Figures 12--14 comparison plot,
equilibrium-fraction table, and machine-readable validation report:

```bash
python -u python_scripts/physics_ice/process_evidence/charge_exchange_ctmc/benchmarking/benchmark_carbon_charge_exchange_ctmc.py \
  cross_sections/carbon_charge_exchange
```

This writes only PNG and tabular/report products under
...
The plot reproduces the definitions in Liamsuwan and Nikjoo (2013), Figures
12--14. The report independently checks table structure and identities,
adaptive impact/energy convergence, the separate 95% Monte Carlo confidence
limit, energy conservation, primitive-probability bounds, exact reintegration
...
run the benchmark with `--allow-unrefined-reference`. This explicitly records
`fixed_grid_reference`, reports the absent adaptive gate as informational, and
can never mark the product production-release ready.

The published calculation does not specify a numerical acceptance value for
the probability at `bmax`. Supply a separately calculated table with larger
impact cutoffs to close that release gate without inventing one:

```bash
python -u python_scripts/physics_ice/process_evidence/charge_exchange_ctmc/benchmarking/benchmark_carbon_charge_exchange_ctmc.py \
  cross_sections/carbon_charge_exchange \
  --boundary-reference /path/to/expanded_bmax/carbon_charge_exchange_h2o.csv \
  --strict-release
```

Without `--boundary-reference`, the plots and other checks are still written,
but `production_release_ready` remains false and the boundary check is marked
`not_evaluated`. `--strict-release` turns any failed or unevaluated required
gate into a nonzero exit status.
...
### Exact retained carbon paper-reproduction invocation

The complete copy-paste command used for the current 168-shard carbon
fixed-grid campaign, including all numerical controls and its retained output
directory, is versioned in
[`physics_ice/WORKFLOW_COMMANDS.md`](../../../WORKFLOW_COMMANDS.md). Unlike the
generic example above, that section contains no placeholders. Its boundary
values are explicitly labelled convergence-domain choices because Paper IV
does not publish them; reproducing the command does not by itself establish
agreement with the paper or pass the adaptive and physical-validation gates.
...
The checkpoint also stores completed chunks that lie beyond a temporarily
unfinished channel prefix. A one-second parent heartbeat tests the 300-second
checkpoint clock even when no worker returns, so a single long trajectory
cannot leave the other workers' completed chunks only in memory.
...
Checkpoint compatibility is provided specifically among the audited legacy
unbounded, 25,000,000-step, and 100,000,000-step settings. Any other work
budget remains in the configuration signature because an earlier cap can
select a tighter numerical retry.

The autonomous physical-time kernel permits local steps below machine epsilon
when their products with the state derivatives remain representable. Elapsed
time is retained by compensated summation. Do not restore an absolute local
step floor: the close-encounter regression tests demonstrate that it falsely
rejects valid finite-periapsis trajectories.
...
python -u python_scripts/physics_ice/generate_carbon_charge_exchange_ctmc.py \
  --prepare-rebalance-from-shards OLD_COUNT \
  --source-ownership-manifest OLD_MANIFEST.npz \
  --shard-count NEW_COUNT \
  --start-separation-au "${START_SEPARATION_AU}" \
  --target-bmax-au "${TARGET_BMAX_AU}" \
  --loss-bmax-au "${LOSS_BMAX_AU}"
```

Use this only after the source writers have been stopped or have finished. A
reasonable production trigger is: the base-grid run is above 95% of committed
trajectories, fewer than roughly one quarter of the original shards remain
active, and no source checkpoint has advanced for at least one checkpoint
interval. For the final tail, choose at most one target shard per unfinished
probability point, capped by the queue's maximum PBS array size and the
available nodes. The new merge job must depend on every replacement array, and
adaptive refinement should remain dependent on that merge.

This freeze/rebalance/restart sequence can be automated with
`pbs/supervise_charge_exchange_ctmc_tail.pbs`. Submit the one-core supervisor
with the exact source array-parent IDs and the same boundary/grid environment
used for the compute arrays. It measures committed trajectories from the
checkpoints. At the 95%/one-quarter trigger it deletes every named source
array, waits until all writers have exited and every checkpoint has remained
unchanged for the configured freeze interval, invokes the audited rebalance
operation, and submits replacement arrays with the immutable ownership
manifest. Its atomic JSON state permits the supervisor itself to resume, and
the cycle may repeat for a later tail. It refuses to continue if all PBS jobs
end before the grid is complete; deterministic trajectory failures must still
be investigated rather than hidden by automatic resubmission.

The supervisor must receive `EXTRA_ARGS` containing the exact energy, charge,
impact-grid, trajectory-count, and output-directory settings. For example:

```bash
qsub -v \
ATOM=carbon,SOURCE_JOB_IDS=ARRAY1\;ARRAY2,SOURCE_SHARDS=168,\
TARGET_SHARDS=168,OUTPUT_DIR=/absolute/run/path,\
START_SEPARATION_AU="$START_SEPARATION_AU",\
TARGET_BMAX_AU="$TARGET_BMAX_AU",LOSS_BMAX_AU="$LOSS_BMAX_AU",\
EXTRA_ARGS="$EXTRA_ARGS",SHARD_MEMORY=8gb,TAIL_QUEUES=idle \
pbs/supervise_charge_exchange_ctmc_tail.pbs
```

The compute-memory request is independent of the one-core supervisor. Measure
it from completed/running PBS jobs and retain headroom; do not infer physical
accuracy from the memory setting.
...
The diagnostic records the numerical-integrator policy version. The tail
supervisor automatically resubmits an old-policy failure against the current
predeclared DOP853/Sundman fallback ladder, preserving the checkpoint,
trajectory identity, physical equations, tolerances, boundary, and 0.1%
energy gate. Recovery attempts are stateful and bounded (three by default),
so scheduler loss cannot create an infinite submission loop. If the exact
trajectory exhausts the current policy, the supervisor records it under
`quarantined_failures` and refuses to repeat it automatically; that case needs
a reviewed numerical-method change and a policy-version increment. Never hide
it by increasing `--max-failure-fraction` or weakening the energy gate.
...
in [`PARALLEL_EXECUTION.md`](PARALLEL_EXECUTION.md).
