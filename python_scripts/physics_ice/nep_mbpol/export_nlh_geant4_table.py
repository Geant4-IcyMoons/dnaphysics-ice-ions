...
SCHEMA_VERSION = 1
SUPPORTED_GEANT4_PROJECTILES = ("C",)
...
def _pair_record(table: AdaptiveKernelTable, projectile: str, target: str) -> dict[str, Any]:
    ...
    return {
        "target": target,
        "target_mass_c2_ev": ISOTOPE_MASS_U[target] * ATOMIC_MASS_UNIT_C2_EV,
...
def export_table(
    ...
    projectile = canonical_element(projectile)
    if projectile not in SUPPORTED_GEANT4_PROJECTILES:
        raise ValueError(
            "Only carbon is released for Geant4 transport; extend and validate "
            "the model before exporting another projectile."
        )
    ...
    lines = [
        ...
        f"# projectile_mass_c2_eV {ISOTOPE_MASS_U[projectile] * ATOMIC_MASS_UNIT_C2_EV:.17g}",
    ...
    manifest = {
        ...
        "projectile_mass_c2_ev": ISOTOPE_MASS_U[projectile]
        * ATOMIC_MASS_UNIT_C2_EV,
        ...
        "source_manifest": str(Path("..") / table.manifest_path.name),
        "source_manifest_sha256": _sha256(table.manifest_path),
        "source_csv": str(Path("..") / table.csv_path.name),
        ...
        "scope": (
            "threshold-defined NLH carbon hard elastic scattering on independent "
            "H and O atoms in pure water ice"
        ),
        "excluded_physics": [
