...
STRUCTURE_PATH = HERE / "hexagonal_ih_100K_experimental" / "seed1000_final.xyz.gz"
PNG_PATH = HERE / "hexagonal_ice_structure.png"
...
def plot_ice_structure_comparison(
    ...
    amorphous_path: Path,
    png_path: Path,
    cubic_path: Path | None = None,
) -> Path:
    """Render matching views in Ih, optional Ic, and LDA row order."""
    _configure_style()
    crop_side_angstrom = 14.0
    structures = [("Hexagonal ice Ih, 100 K", hexagonal_path)]
    if cubic_path is not None:
        structures.append(("Cubic ice Ic, 100 K", cubic_path))
    structures.append(("Amorphous LDA, 80 K", amorphous_path))
    scenes = [_comparison_scene(path, crop_side_angstrom) for _, path in structures]
    row_count = len(scenes)
    fig = plt.figure(figsize=(AASTEX_FULL_WIDTH_IN, row_count * THREE_PANEL_ROW_HEIGHT_IN))
    grid = fig.add_gridspec(row_count, 3, wspace=0.035, hspace=0.055)
    labels = [tuple(f"({chr(97 + 3 * row + col)})" for col in range(3))
    ...
    for row, scene in enumerate(scenes):
        ...
        bottom_label = r"$x$ (Å)" if row == row_count - 1 else None
        ...
        _draw_projection(
            ...
            draw_scale_bar=row == row_count - 1, x_label=bottom_label,
        )
    ...
    fig.subplots_adjust(left=0.055, right=0.995, bottom=0.21 / row_count, top=0.985)
    for (label, _), frame in zip(structures, row_frames):
        bounds = frame.get_position()
        fig.text(0.008, 0.5 * (bounds.y0 + bounds.y1), label, rotation=90,
                 ha="left", va="center")
    png_path.parent.mkdir(parents=True, exist_ok=True)
    temporary = png_path.with_name(png_path.stem + f".tmp-{os.getpid()}.png")
    try:
    ...
    records = [{"label": label, "path": str(path.resolve()),
                "sha256": hashlib.sha256(path.read_bytes()).hexdigest(),
                "displayed_waters": len(scene[0])}
               for (label, path), scene in zip(structures, scenes)]
    png_path.with_suffix(".json").write_text(json.dumps({
        "crop_side_angstrom": crop_side_angstrom,
        "png_sha256": hashlib.sha256(png_path.read_bytes()).hexdigest(),
        "rows": records,
        "note": "Visualization provenance; rendering does not establish structure acceptance.",
    }, indent=2) + "\n")
    print(f"Saved: {png_path} ({row_count} rows, {crop_side_angstrom:g} A cubes)")
    return png_path
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--cubic-structure", type=Path,
        help="Insert a 100 K Ic structure between the Ih and LDA rows.",
    )
    return parser
...
if __name__ == "__main__":
    ...
    if arguments.cubic_structure is not None and arguments.amorphous_structure is None:
        raise SystemExit("--cubic-structure requires --amorphous-structure")
    if arguments.amorphous_structure is None:
        ...
        plot_ice_structure_comparison(
            ...
            cubic_path=arguments.cubic_structure,
            png_path=arguments.output_stem.with_suffix(".png"),
        )
