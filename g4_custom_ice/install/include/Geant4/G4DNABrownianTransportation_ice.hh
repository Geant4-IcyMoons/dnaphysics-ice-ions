...
#ifndef G4ITBROWNIANTRANSPORTATION_ICE_H
#define G4ITBROWNIANTRANSPORTATION_ICE_H
...
class G4DNABrownianTransportation_ice : public G4ITTransportation
{
public:
  G4DNABrownianTransportation_ice(const G4String& aName =
                              "DNABrownianTransportation_ice",
                              G4int verbosityLevel = 0);
  ~G4DNABrownianTransportation_ice() override;
  G4DNABrownianTransportation_ice(const G4DNABrownianTransportation_ice&) = delete;
  G4DNABrownianTransportation_ice& operator=(const G4DNABrownianTransportation_ice&) = delete;
...
inline void G4DNABrownianTransportation_ice::SetBrownianAction(G4BrownianAction* brownianAction)
{
  fpBrownianAction = brownianAction;
}

inline void G4DNABrownianTransportation_ice::SetUserBrownianAction(G4VUserBrownianAction* brownianAction)
{
  fpUserBrownianAction = brownianAction;
}

// Function to set temperature profile for ice transport (to be called from user application)
void SetIceTemperatureProfile(const std::vector<G4double>& zRanges, const std::vector<G4double>& zTemperatures);

// Function to get temperature at a given position (for use by other ice processes)
G4double GetIceTemperatureAtPosition(const G4ThreeVector& position);

#endif // G4ITBROWNIANTRANSPORTATION_ICE_H
