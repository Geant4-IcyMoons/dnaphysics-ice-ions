...
//  G4EmDNABuilder_ice
//
//  This is a custom Geant4-DNA physics builder for simulating electron
//  and ion interactions in ice (solid H2O), as opposed to the standard
//  Geant4-DNA builder which is designed for liquid water.
//
//  Key differences from the original (non-_ice) builder:
//    - Registers and configures custom elastic, excitation, and vibrational
//      excitation models for ice, including the Michaud models and custom
//      cross section data for ice.
//    - Uses G4DNAVibExcitation_ice, G4DNAMichaudElasticModel, and
//      G4DNAMichaudExcitationModel instead of the default Sanche or Champion
//      models for vibrational and elastic processes.
//    - Ensures all relevant processes use physics and data appropriate for ice.
//    - May set different energy limits, process orderings, or region assignments
//      compared to the standard builder.
//
//  Use this builder in your physics list to enable accurate Geant4-DNA
//  simulations for icy environments (e.g., planetary ices, astrophysical ices).
// ---------------------------------------------------------------------

// Author: V. Ivanchenko (original), customized for ice by [Your Name/Group]


#include "G4EmDNABuilder_ice.hh"
...
void G4EmDNABuilder_ice::ConstructDNAParticles()
{
  // standard particles
...
void 
G4EmDNABuilder_ice::ConstructStandardEmPhysics(const G4double emin_elec,
...
void G4EmDNABuilder_ice::StandardHadronPhysics(G4ParticleDefinition* part,
...
void 
G4EmDNABuilder_ice::ConstructDNAElectronPhysics(const G4double emaxDNA,
                                            const G4int opt,
                                            const G4bool fast,
                                            ...
                                            const G4Region* reg)
{
  ...
  if(4 == opt) {
    ...
    elast2 = new G4DNAMichaudElasticModel();
  } else {
    // elast = new G4DNAChampionElasticModel();
    elast = new G4DNAMichaudElasticModel();
  }
  ...
  auto theDNAVibExc = FindOrBuildVibExcitation(part, "e-_G4DNAVibExcitation_ice");
    auto modS = new G4DNAMichaudExcitationModel();
    ...
    auto modM = new G4DNAMichaudAttachmentModel();
...
void 
G4EmDNABuilder_ice::ConstructDNAProtonPhysics(const G4double e1DNA,
...
void 
G4EmDNABuilder_ice::ConstructDNAIonPhysics(const G4double emaxIonDNA,
                                       const G4bool stationary,
                                       const G4Region* reg)
{
...
void
G4EmDNABuilder_ice::ConstructDNALightIonPhysics(G4ParticleDefinition* part,
                                            const G4int charge,
                                            const G4int opt,
                                            const G4double emaxIonDNA,
					    const G4bool,
...
G4DNAElectronSolvation* G4EmDNABuilder_ice::FindOrBuildElectronSolvation()
{
...
G4DNAElastic* 
G4EmDNABuilder_ice::FindOrBuildElastic(G4ParticleDefinition* part,
                                   const G4String& name)
{
...
G4DNAExcitation* 
G4EmDNABuilder_ice::FindOrBuildExcitation(G4ParticleDefinition* part,
                                      const G4String& name)
{
...
G4DNAVibExcitation_ice* 
G4EmDNABuilder_ice::FindOrBuildVibExcitation(G4ParticleDefinition* part,
                                         const G4String& name)
{
  auto p = G4PhysListUtil::FindProcess(part, fLowEnergyVibrationalExcitation);
  G4DNAVibExcitation_ice* ptr = dynamic_cast<G4DNAVibExcitation_ice*>(p);
  if(nullptr == ptr) { 
    ptr = new G4DNAVibExcitation_ice(name);
    G4PhysicsListHelper* ph = G4PhysicsListHelper::GetPhysicsListHelper();
    ph->RegisterProcess(ptr, part);
    ptr->SetEmModel(new G4DummyModel());
  }
...
G4DNAIonisation* 
G4EmDNABuilder_ice::FindOrBuildIonisation(G4ParticleDefinition* part,
                                      const G4String& name)
{
...
G4DNAAttachment* 
G4EmDNABuilder_ice::FindOrBuildAttachment(G4ParticleDefinition* part,
                                      const G4String& name)
{
...
G4DNAChargeDecrease*
G4EmDNABuilder_ice::FindOrBuildChargeDecrease(G4ParticleDefinition* part,
                                          const G4String& name)
{
...
G4DNAChargeIncrease*
G4EmDNABuilder_ice::FindOrBuildChargeIncrease(G4ParticleDefinition* part,
                                          const G4String& name)
{
...
G4LowECapture*
G4EmDNABuilder_ice::FindOrBuildCapture(const G4double elim, G4ParticleDefinition* part)
{
...
void G4EmDNABuilder_ice::FindOrBuildNuclearStopping(G4ParticleDefinition* part,
                                                const G4double elim)
{
