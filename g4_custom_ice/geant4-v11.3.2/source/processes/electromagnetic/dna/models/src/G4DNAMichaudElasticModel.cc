...
G4DNAMichaudElasticModel::
G4DNAMichaudElasticModel(const G4ParticleDefinition*, const G4String& nam) :
    G4VEmModel(nam) 
{
  ...
  if (verboseLevel > 0)
  {
    G4cout << "Michaud Elastic model is constructed "
           << G4endl
           << "Energy range: "
...
G4DNAMichaudElasticModel::~G4DNAMichaudElasticModel()
{
...
void G4DNAMichaudElasticModel::Initialise(const G4ParticleDefinition* particle,
                                           const G4DataVector& /*cuts*/)
{
#ifdef MICHAUD_VERBOSE
  if (verboseLevel > 3)
  {
    G4cout << "Calling G4DNAMichaudElasticModel::Initialise()" << G4endl;
  }
#endif
  
  if(particle->GetParticleName() != "e-")
  {
    G4Exception("G4DNAMichaudElasticModel::Initialise",
                "em0002",
                FatalException,
  ...
  {
    G4cout << "G4DNACMichaudElasticModel: low energy limit increased from "
           << LowEnergyLimit()/eV << " eV to " << 2. << " eV"
           << G4endl;
  ...
  {
    G4cout << "G4DNAMichaudElasticModel: high energy limit decreased from "
           << HighEnergyLimit()/eV << " eV to " << 100.*eV << " eV"
           << G4endl;
    SetHighEnergyLimit(100.*eV);
  }
  ...
  if (path == nullptr)
  {
    G4Exception("G4MichaudElasticModel::Initialise",
                "em0006",
                FatalException,
  ...
  eFullFileName << path << "/dna/sigmadiff_cumulated_elastic_e_michaud_hp.dat";
  ...
  if (verboseLevel>0)
  {
    if (verboseLevel > 1)
    {
      G4cout << "Loaded cross section files for Michaud Elastic model" << G4endl;
    }

    G4cout << "Michaud Elastic model is initialized " << G4endl
           << "Energy range: "
           << LowEnergyLimit() / eV << " eV - "
           << HighEnergyLimit() / MeV << " MeV"
           << G4endl;
                      ...
                      G4double)
{
  ...
  if (verboseLevel > 3)
  {
   G4cout << "Calling CrossSectionPerVolume() of G4DNAMichaudElasticModel"
          << G4endl;
  }
#endif
  ...
  if (verboseLevel > 2)
  {
    ...
    G4cout << "=== G4DNAMichaudElasticModel - XS INFO START" << G4endl;
    G4cout << "=== Kinetic energy(eV)=" << ekin/eV << " particle : " << p->GetParticleName() << G4endl;
    G4cout << "=== Cross section per water molecule (cm^2)=" << sigma/cm/cm << G4endl;
    G4cout << "=== Cross section per water molecule (cm^-1)=" << sigma*waterDensity/(1./cm) << G4endl;
    G4cout << "=== G4DNAMichaudElasticModel - XS INFO END" << G4endl;
  }
#endif
...
void G4DNAMichaudElasticModel::SampleSecondaries(std::vector<G4DynamicParticle*>* /*fvect*/,
                                                  ...
                                                  G4double)
{
  ...
  if (verboseLevel > 3)
  {
    G4cout << "Calling SampleSecondaries() of G4DNAMichaudElasticModel" << G4endl;
  }
#endif
...
G4double G4DNAMichaudElasticModel::Theta(G4double k,
                                          G4double integrDiff)
{
  G4double theta = 0.;
  G4double valueT1 = 0;
...
G4double G4DNAMichaudElasticModel::LinLogInterpolate(G4double e1,
                                                      G4double e2,
                                                      G4double e,
                                                      G4double xs1,
                                                      G4double xs2)
...
G4double G4DNAMichaudElasticModel::LinLinInterpolate(G4double e1,
                                                      G4double e2,
                                                      G4double e,
                                                      G4double xs1,
                                                      G4double xs2)
...
G4double G4DNAMichaudElasticModel::LogLogInterpolate(G4double e1,
                                                      G4double e2,
                                                      G4double e,
                                                      G4double xs1,
                                                      G4double xs2)
...
G4double G4DNAMichaudElasticModel::QuadInterpolator(G4double e11,
                                                     G4double e12,
                                                     G4double e21,
                                                     G4double e22,
                                                     G4double xs11,
...
G4double G4DNAMichaudElasticModel::RandomizeCosTheta(G4double k)
{
...
void G4DNAMichaudElasticModel::SetKillBelowThreshold(G4double)
{
  G4ExceptionDescription errMsg;
  errMsg << "The method G4DNAMichaudElasticModel::SetKillBelowThreshold is deprecated";
  
  G4Exception("G4DNAMichaudElasticModel::SetKillBelowThreshold",
              "deprecated",
              JustWarning,
              errMsg);
}
