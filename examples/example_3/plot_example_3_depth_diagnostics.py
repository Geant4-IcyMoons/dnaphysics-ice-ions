#!/usr/bin/env python3
"""Depth diagnostics for example_3.root."""
...
def main() -> None:
    ...
    default_root = script_dir / "outputs" / "example_3.root"
    default_out = plot_dir / "example_3_depth_diagnostics.png"

    parser = argparse.ArgumentParser(description="Plot depth diagnostics for example_3.root")
    parser.add_argument("--root", default=str(default_root), help="Path to example_3.root")
    ...
    primary_mev_rounded = np.round(primary_mev, 9)
    unique_energies = np.unique(primary_mev_rounded)

    energy_groups: list[tuple[float, np.ndarray]] = []
    for energy_key in np.sort(unique_energies):
        mask = primary_mev_rounded == energy_key
        ids = event_ids[mask]
        energy_groups.append((float(np.mean(primary_mev[mask])), ids))
    ...
    colors = plt.get_cmap("tab10")(np.linspace(0.0, 1.0, len(energy_groups)))
    ...
    for color, (energy_mev, grouped_event_ids) in zip(colors, energy_groups):
        event_mask = np.isin(step_event, grouped_event_ids)
        ...
        n_events = max(int(grouped_event_ids.size), 1)

        total_profile = binned_sum(z_mm[in_slab], deposited_mev[in_slab], bins_mm) / n_events
        electron_profile = (
            binned_sum(
        ...
        label = f"{format_mev(energy_mev)} ({n_events} events)"
        ...
        ax_ke.plot(centers_mm, primary_ke_profile, color=color, label=f"{format_mev(energy_mev)} primary")
        if np.any(np.isfinite(secondary_ke_profile)):
            ax_ke.plot(
                centers_mm,
                secondary_ke_profile,
                color=color,
                linestyle="--",
                label=f"{format_mev(energy_mev)} secondary",
            )
        ax_cumulative.plot(centers_mm, cumulative, color=color, label=label)

    ax_electron_dep.set_title("Electron Deposited Energy Per Primary")
    ax_electron_dep.set_ylabel("Deposited energy per bin per primary (MeV)")
    ax_electron_dep.grid(True, alpha=0.3)

    ax_total_dep.set_title("All-Particle Deposited Energy Per Primary")
    ax_total_dep.set_ylabel("Deposited energy per bin per primary (MeV)")
    ...
    fig.suptitle("Example 3: Depth diagnostics", fontsize=14)
