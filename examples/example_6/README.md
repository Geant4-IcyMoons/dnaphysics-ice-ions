...
`example_6` is the water W-value Geant4-IcyMoons example.
...
- macro: [`example_6.mac`](example_6.mac)
- runner: [`run_example_6.sh`](run_example_6.sh)
- plotter: [`plot_example_6_wvalue.py`](plot_example_6_wvalue.py)
...
- sets `DNA_PHYSICS=water`
- sets `DNA_ROOT_BASENAME=example_6`
- sets `DNA_WVALUE_FILE` to the requested text filename
- sets `DNA_NTUPLE_FILES=0`
- launches `dnaphysics` on [`example_6.mac`](example_6.mac)

Expected outputs:

- `outputs/example_6.root`
- `outputs/example_6_wvalue.txt`
...
python3 examples/example_6/plot_example_6_wvalue.py \
  ...
  --out "plots/examples/example_6/example_6_wvalue.png"
```
...
This example follows the current water W-value workflow, which is intended for the Geant4-DNA water model family. In the present setup, the reliable W-value range extends up to about `1 MeV`, and this example therefore stops at `1 MeV`.
