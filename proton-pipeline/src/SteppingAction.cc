...
// Particle identification
  ...
  */
  if (processName != "Transportation") {
    ...
    rec.model = modelName;
    // Derive a proton-specific channel label for logging/printing.
    int chanIdx = -1;
    G4double chanMicroXS_cm2 = -1.0;
    if (processName.find("Excitation") != std::string::npos) {
      const bool isIceProtonEmfi =
          (modelName.find("Emfietzoglou_iceProtonExcitationModel") != std::string::npos);
      int levelIdx = -1;
      if (isIceProtonEmfi) {
        levelIdx = G4DNAEmfietzoglou_iceProtonExcitationModel::GetLastExcitationIndex();
        chanMicroXS_cm2 = G4DNAEmfietzoglou_iceProtonExcitationModel::GetLastPartialSigma_cm2();
      }
      if (levelIdx >= 0) {
        chanIdx = levelIdx;
      ...
      const bool isIceProtonEmfi =
          (modelName.find("Emfietzoglou_iceProtonIonisationModel") != std::string::npos);
      int shellIdx = -1;
      if (isIceProtonEmfi) {
        shellIdx = G4DNAEmfietzoglou_iceProtonIonisationModel::GetLastShellIndex();
        chanMicroXS_cm2 = G4DNAEmfietzoglou_iceProtonIonisationModel::GetLastPartialSigma_cm2();
      }
      if (shellIdx >= 0) {
        chanIdx = shellIdx;
        std::ostringstream ch;
