...
void DetectorMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  if (command == fpPhysCmd) {
    auto* phys = dynamic_cast<PhysicsList_Proton*>(fpPhysList);
    if (phys != nullptr) {
      phys->AddPhysics(newValue);
    } else {
      G4cout << "### DetectorMessenger Warning: addPhysics is only supported for PhysicsList_Proton."
             << G4endl;
    }
  }
  ...
  if (command == fpTrackingCutCmd) {
    ...
    if (phys != nullptr) {
      ...
      G4cout << "### DetectorMessenger Warning: addIonsTrackingCut is only supported for PhysicsList_Proton."
             << G4endl;
    }
  }
